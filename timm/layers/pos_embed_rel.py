...
def resize_rel_pos_bias_table_levit(
    ...
    if L1 != L2:
        ...
        relative_position_bias_table_resized = F.interpolate(
            position_bias_table.permute(1, 0).view(1, nH1, S1, S1), size=(S2, S2), mode=interpolation, antialias=antialias)
...
class RelPosMlp(nn.Module):
    ...
    def __init__(self, window_size, num_heads=8, hidden_dim=128, prefix_tokens=0, mode='cr', pretrained_window_size=(0, 0)):
        ...
        self.register_buffer("rel_coords_log", gen_relative_log_coords(window_size, pretrained_window_size, mode=mode),
                             persistent=False)
...
class RelPosBiasTf(nn.Module):
    ...
    def get_bias(self) -> torch.Tensor:
        ...
        return reindex_2d_einsum_lookup(self.relative_position_bias_table, self.window_size[0], self.window_size[1],
                                        self.height_lookup, self.width_lookup)
