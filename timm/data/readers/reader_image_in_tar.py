    ...
    for i, ti in enumerate(tf):
        ...
        if ext == '.tar':
            with tarfile.open(fileobj=tf.extractfile(ti), mode='r|') as ctf:
                child_info = dict(name=ti.name, path=os.path.join(parent_info['path'], name), ti=ti, children=[], samples=[])
...
def extract_tarinfos(root, class_name_to_idx: Optional[Dict] = None, cache_tarinfo: Optional[bool] = None,
                     extensions: Optional[Union[List, Tuple, Set]] = None, sort: bool = True):
    ...
    if os.path.exists(cache_path):
        ...
        for i, fn in enumerate(tar_filenames):
            ...
            with tarfile.open(fn, mode='r|') as tf:  # tarinfo scans done in streaming mode
                ...
                _logger.debug(f'{i}/{num_tars}. Extracted tarinfos from {fn}. {num_children} children, {num_samples} samples.')
...
class ReaderImageInTar(Reader):
    ...
    def __init__(self, root, class_map='', cache_tarfiles=True, cache_tarinfo=None):
        ...
        self.root = root
        self.samples, self.targets, self.class_name_to_idx, tarfiles = extract_tarinfos(self.root,
